...
plot_lasagna_1subject <- function(data, plottype = c('unsorted', 'timesorted'), limits = c(50, 500), midpoint = 105, lower = 70, upper = 140, dt0 = NULL, inter_gap = 60, tz = ""){
  ...
  p = data_l%>%
    ggplot(aes(x = hour, y = day, fill = glucose)) + geom_tile()  + ylab("Day") + ggtitle(paste0(subject, title, "")) + xlab("Hour") + xlim(c(0, 24)) + scale_fill_gradientn(colors = c("#3182bd", "#deebf7", "white", "#fee0d2", "#de2d26"), na.value = "grey50", values = scales::rescale(c(limits[1], lower, midpoint, upper, limits[2])), limits = limits)
...
plot_lasagna <- function(data, datatype = c("all", "average"), plottype_average = c('unsorted', 'timesorted'), maxd = 14, limits = c(50, 500), midpoint = 105, lower = 70, upper = 140, dt0 = NULL, inter_gap = 60, tz = ""){
  ...
  if (datatype == "average"){
    ...
    p = data_l%>%
      ggplot(aes(x = hour, y = subject, fill = glucose)) + geom_tile() + ylab("Subject") + ggtitle(paste0("24 hours averages for all subjects", title, "")) + xlab("Hour") + xlim(c(0, 24)) + scale_fill_gradientn(colors = c("#3182bd", "#deebf7", "white", "#fee0d2", "#de2d26"), na.value = "grey50", values = scales::rescale(c(limits[1], lower, midpoint, upper, limits[2])), limits = limits)
    return(p)
  }else{
